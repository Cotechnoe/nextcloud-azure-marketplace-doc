...
tags: ["azure-marketplace", "anti-hallucination", "ia", "sources", "partner-center", "documentation", "référentiel"]
...
| **Complémentaire de** | ADR-002 (contrainte générale IA), ADR-801 (stratégie documentation) |
...
| 7 | Processus de certification | `/vm/#the-virtual-machine-offer-certification-process` | Causes communes d'échec |
| 8 | Outils de test VM | `/vm/#virtual-machine-test-tools-for-marketplace-demo` | AMAT (Azure Marketplace Assessment Tool) |
| 9 | Sécurité VM & containers | `/vm/#securing-virtual-machines-and-containers-for-certification` | PDF + vidéo certification |
| 10 | Déprécier / restaurer images | `/vm/#deprecating-and-restoring-virtual-machine-images` | Cycle de vie images |
...
>
> Note : Le diagramme ci-dessous décrit le processus global de publication. Les détails d'implémentation (étapes 2 et 5) sont gérés par le dépôt build `Cotechnoe/nextcloud-marketplace`. Cette section sert de référence contextuelle pour les rédacteurs de documentation.
...
│  └── Mise à jour : nouveau build d'image → nouvelle version gallery │
└─────────────────────────────────────────────────────────────────────┘
```
...
- ✅ Référencer cet ADR (ADR-802) dans tout ADR ou contenu traitant de Marketplace
...
| [ADR-801](./801-BIZ-strategie-documentation-marketplace.md) | Stratégie Documentation Marketplace | ADR-802 fournit les **sources** que ADR-801 requiert pour rédiger avec précision |
| [ADR-803](./803-BIZ-titre-offre-marketplace-conformite-marque.md) | Titre offre et conformité marque | ADR-802 fournit les sources de conformité Marketplace sur lesquelles ADR-803 s'appuie |
...
- Réduit le risque d'erreurs de contenu dans la documentation publiée
...
- Le site Mastering the Marketplace est v3.0.0 (2026-05-25) — les modules vidéo peuvent devenir périmés avant les docs MS Learn
